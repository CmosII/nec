...
- (void)itemsWillAppearFromButton:(UIButton *) button withFrame:(CGRect)frame inView:(UIView *)view {
	...
	int centerX = frame.origin.x + (frame.size.height/2) - 15; //9 is middle
	...
	while (currentItem <= itemCount) {
		float radians = (angle * (currentItem - 1 + 3.15) + start) * (M_PI/180);
...
- (void)shouldRotateButton:(UIButton *)button forDuration:(float)duration forwardDirection:(BOOL)direction {
	...
	//do a 360 turn for each object being flung
	float totalDuration = M_PI * [self.items count] * 2;
	if (!direction) {
